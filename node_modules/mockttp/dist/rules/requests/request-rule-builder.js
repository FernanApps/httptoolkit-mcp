...
class RequestRuleBuilder extends base_rule_builder_1.BaseRuleBuilder {
    ...
    constructor(methodOrAddRule, path, addRule) {
        ...
        if (method === undefined && path === undefined) {
            ...
            if (path instanceof RegExp) {
                ...
                this.matchers.push(new matchers_1.FlexiblePathMatcher(path));
            }
     ...
     */
    delay(ms) {
        this.steps.push(new request_step_definitions_1.DelayStep(ms));
        return this;
    }
    /**
     ...
     */
    waitForRequestBody() {
        this.steps.push(new request_step_definitions_1.WaitForRequestBodyStep());
        return this;
    }
    /**
    ...
    addWebhook(url, events) {
        this.steps.push(new request_step_definitions_1.WebhookStep(url, events ?? ['request', 'response']));
        return this;
    }
    thenReply(status, dataOrMessage, dataOrHeaders, headersOrTrailers, trailers) {
        ...
        this.steps.push(new request_step_definitions_1.FixedResponseStep(status, statusMessage, data, headers, trailers));
        const rule = {
    ...
    thenJson(status, data, headers = {}) {
        ...
        this.steps.push(new request_step_definitions_1.FixedResponseStep(status, undefined, jsonData, headers));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenCallback(callback) {
        this.steps.push(new request_step_definitions_1.CallbackStep(callback));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenStream(status, stream, headers) {
        this.steps.push(new request_step_definitions_1.StreamStep(status, stream, headers));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenFromFile(status, pathOrMessage, pathOrHeaders, headers) {
        ...
        this.steps.push(new request_step_definitions_1.FileStep(status, statusMessage, path, headers));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenPassThrough(options) {
        this.steps.push(new request_step_definitions_1.PassThroughStep(options));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
     ...
     * @category Responses
     */
    async thenForwardTo(target, options = {}) {
        ...
        this.steps.push(new request_step_definitions_1.PassThroughStep({
            ...options,
            transformRequest: {
     ...
     */
    thenCloseConnection() {
        this.steps.push(new request_step_definitions_1.CloseConnectionStep());
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
     ...
     */
    thenResetConnection() {
        this.steps.push(new request_step_definitions_1.ResetConnectionStep());
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
     ...
     */
    thenTimeout() {
        this.steps.push(new request_step_definitions_1.TimeoutStep());
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenSendJsonRpcResult(result) {
        this.steps.push(new request_step_definitions_1.JsonRpcResponseStep({ result }));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
    ...
    thenSendJsonRpcError(error) {
        this.steps.push(new request_step_definitions_1.JsonRpcResponseStep({ error }));
        const rule = {
            ...this.buildBaseRuleData(),
            steps: this.steps
        };
