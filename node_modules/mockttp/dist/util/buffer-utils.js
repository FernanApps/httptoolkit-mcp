...
const MAX_BUFFER_SIZE = util_1.isNode
    ...
    : Infinity;
const asBuffer = (input) => buffer_1.Buffer.isBuffer(input)
    ? input
    : typeof input === "string"
        ? buffer_1.Buffer.from(input, 'utf8')
        // Is Array:
        ...
        read(size) {
            ...
            if (!active) {
                if (buffer.failedWith) {
                    ...
                    outputStream.write(buffer_1.Buffer.concat(buffer.currentChunks));
...
const streamToBuffer = (input, maxSize = MAX_BUFFER_SIZE) => {
    ...
    const bufferPromise = new Promise((resolve, reject) => {
        ...
        if (input.readableEnded)
            return resolve(buffer_1.Buffer.from([]));
        if (input.readableAborted)
            return setImmediate(failWithAbortError);
        let currentSize = 0;
        const onData = (d) => {
