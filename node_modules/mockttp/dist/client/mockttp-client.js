...
class MockttpClient extends mockttp_1.AbstractMockttp {
    ...
    messageBodyDecoding;
    adminClient;
    requestBuilder; // Set once server has started.
    constructor(options = {}) {
    ...
    async start(port) {
        ...
        this.requestBuilder = new mockttp_admin_request_builder_1.MockttpAdminRequestBuilder(this.adminClient.schema, { messageBodyDecoding: this.messageBodyDecoding });
    }
    stop() {
        return this.adminClient.stop();
    }
    addRequestRules = async (...rules) => {
        return this._addRequestRules(rules, false);
    };
    setRequestRules = async (...rules) => {
        return this._addRequestRules(rules, true);
    };
    addWebSocketRules = async (...rules) => {
        return this._addWsRules(rules, false);
    };
    setWebSocketRules = async (...rules) => {
        return this._addWsRules(rules, true);
    };
    _addRequestRules = async (rules, reset) => {
        if (!this.requestBuilder)
            throw new Error('Cannot add rules before the server is started');
        const { adminStream } = this.adminClient;
        return this.adminClient.sendQuery(this.requestBuilder.buildAddRulesQuery('http', rules, reset, adminStream));
    };
    _addWsRules = async (rules, reset) => {
        if (!this.requestBuilder)
            throw new Error('Cannot add rules before the server is started');
        const { adminStream } = this.adminClient;
        return this.adminClient.sendQuery(this.requestBuilder.buildAddRulesQuery('ws', rules, reset, adminStream));
    };
