...
class AbstractMockttp {
    ...
    addRequestRule = (rule) => this.addRequestRules(rule).then((rules) => rules[0]);
    addWebSocketRule = (rule) => this.addWebSocketRules(rule).then((rules) => rules[0]);
    forAnyRequest() {
    ...
    forAnyWebSocket(url) {
        return new websocket_rule_builder_1.WebSocketRuleBuilder(url, this.addWebSocketRule);
    }
}
